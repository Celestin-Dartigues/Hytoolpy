...
def trial_multi(t, s, models, npoints):
    """
    TRIAL_MULTI - Display data and multiple model solutions (log-log + semi-log).
    ...
    models : list of tuples
        ...
        name  = nom du modèle (ex: 'htj', 'grf'…)
        label = nom affiché dans la légende
        color = couleur du tracé
    t, s : array
        Données expérimentales
    npoints : int
        Points utilisés pour la dérivée logarithmique
    ...
    tplot = np.logspace(np.log10(t[0]), np.log10(t[-1]), 200)

    # correction signe données si besoin
    if np.mean(sd) < 0:
        sd = -sd

    fig, axs = plt.subplots(1, 2, figsize=(12, 5))

    # tracer les données une seule fois
    axs[0].loglog(t, s, 'bo', label='data')
    axs[0].loglog(td, sd, 'rx', label='derivative')
    ...
    for p, name, label, color in models:
        model = name+'.dim(p,tplot)'
        sc = eval(model)
        tdc, dsc = ldiffs(tplot, sc, npoints)
        ...
        # tracer
        axs[0].loglog(tplot, sc, color, label=label)
        axs[0].loglog(tdc, dsc, color+'--')

        axs[1].semilogx(tplot, sc, color, label=label)
        axs[1].semilogx(tdc, dsc, color+'--')
